...
function knockoutWinner(match){if(!match?.completed)return null;const hs=Number(match.homeScore)||0,as=Number(match.awayScore)||0;if(hs>as)return match.home;if(as>hs)return match.away;const hp=Number(match.homePenalties),ap=Number(match.awayPenalties);if(Number.isFinite(hp)&&Number.isFinite(ap)&&hp!==ap)return hp>ap?match.home:match.away;return null}
