...
function calculateTable(fixtures,names){const rows=Object.fromEntries(names.map(name=>[name,{name,p:0,w:0,d:0,l:0,gf:0,ga:0,gd:0,pts:0}]));fixtures.filter(match=>match.completed&&rows[match.home]&&rows[match.away]).forEach(match=>{const home=rows[match.home],away=rows[match.away],hs=Number(match.homeScore)||0,as=Number(match.awayScore)||0;home.p++;away.p++;home.gf+=hs;home.ga+=as;away.gf+=as;away.ga+=hs;if(hs>as){home.w++;home.pts+=3;away.l++}else if(hs<as){away.w++;away.pts+=3;home.l++}else{home.d++;away.d++;home.pts++;away.pts++}});return Object.values(rows).map(row=>({...row,gd:row.gf-row.ga})).sort((a,b)=>b.pts-a.pts||b.gd-a.gd||b.gf-a.gf||a.name.localeCompare(b.name))}

function TableView({fixtures}){return <section className="lc-view"><div className="lc-page-title"><small>LIVE LEAGUE POSITION</small><h1>Points.</h1><p>Win 3 · Draw 1 · Loss 0. Goal totals and positions update only after a match is marked completed.</p></div><div className="lc-table-legend"><span>P Played</span><span>GF Goals for</span><span>GA Goals against</span><span>GD Goal difference</span></div>{Object.entries(groups).map(([group,names])=><article className="lc-table lc-points-table" key={group}><header><b>GROUP {group}</b><span>P&nbsp; W&nbsp; D&nbsp; L&nbsp; GF&nbsp; GA&nbsp; GD&nbsp; PTS</span></header>{calculateTable(fixtures,names).map((row,index)=><div className={index<2?'qualifying':''} key={row.name}><span>{index+1}</span><TeamLogo name={row.name}/><b>{row.name}{index<2&&<em>QUALIFYING</em>}</b><small className="lc-row-stats"><span>{row.p}</span><span>{row.w}</span><span>{row.d}</span><span>{row.l}</span><span>{row.gf}</span><span>{row.ga}</span><span>{row.gd}</span><strong>{row.pts}</strong></small></div>)}</article>)}<QualificationPath/></section>}
