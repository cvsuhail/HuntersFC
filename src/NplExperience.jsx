...
function calculateTable(fixtures,names,live){const rows=Object.fromEntries(names.map(name=>[name,{name,p:0,w:0,d:0,l:0,gf:0,ga:0,gd:0,pts:0,live:false}]));const completed=fixtures.filter(match=>match.completed&&rows[match.home]&&rows[match.away]);const liveActive=['LIVE','HALF TIME'].includes(live?.status)&&rows[live.homeTeam]&&rows[live.awayTeam]&&!completed.some(match=>match.home===live.homeTeam&&match.away===live.away);const matches=liveActive?[...completed,{home:live.homeTeam,away:live.away,homeScore:live.homeScore,awayScore:live.awayScore,provisional:true}]:completed;matches.forEach(match=>{const home=rows[match.home],away=rows[match.away],hs=Number(match.homeScore)||0,as=Number(match.awayScore)||0;home.p++;away.p++;home.gf+=hs;home.ga+=as;away.gf+=as;away.ga+=hs;if(match.provisional){home.live=true;away.live=true}if(hs>as){home.w++;home.pts+=3;away.l++}else if(hs<as){away.w++;away.pts+=3;home.l++}else{home.d++;away.d++;home.pts++;away.pts++}});return Object.values(rows).map(row=>({...row,gd:row.gf-row.ga})).sort((a,b)=>b.pts-a.pts||b.gd-a.gd||b.gf-a.gf||a.name.localeCompare(b.name))}
